...
async def send_test_json(dali):
    ...
    if dali.dlproto == simpledali.DLPROTO_1_0:
        # old style
        streamid = f"{network}_{station}_00_SOH/BZJSON"
    else:
        sid = FDSNSourceId.fromNslc(network, station,"00","SOH")
        streamid = simpledali.fdsnSourceIdToStreamId(sid, simpledali.BZ2_JSON_TYPE)
    ...
    sendResult = await dali.writeBZ2JSON(streamid, hpdatastart, hpdataend, jsonMessage)
    print(f"writeBZ2JSON {streamid} response: {sendResult}")
